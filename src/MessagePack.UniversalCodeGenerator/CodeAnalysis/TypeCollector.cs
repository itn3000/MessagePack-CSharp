    ...
    public class TypeCollector
    {
        ...
        public TypeCollector(string csProjPath, IEnumerable<string> conditinalSymbols, bool disallowInternal, bool isForceUseMap)
        {
            ...
            foreach(var x in compilation.GetNamedTypeSymbols())
            {
                ...
                switch (x.TypeKind)
                {
                    ...
                    case TypeKind.Interface:
                        Console.WriteLine($"name={x.Name},display={x.ToDisplayString()},attrs={string.Join("|", x.GetAttributes().Select(attr => $"{attr.AttributeClass.MetadataName},{attr.AttributeClass.ToDisplayString()}"))},{x.IsAbstract}");
                        break;
                    default:
                        break;
                }
            ...
            targetTypes = compilation.GetNamedTypeSymbols()
                ...
                .Where(x =>
                       ((x.TypeKind == TypeKind.Interface) && x.GetAttributes().Any(x2 => x2.AttributeClass.Name == "Union"))
                    || ((x.TypeKind == TypeKind.Class && x.IsAbstract) && x.GetAttributes().Any(x2 => x2.AttributeClass.Name == "Union"))
                    || ((x.TypeKind == TypeKind.Class) && x.GetAttributes().Any(x2 => x2.AttributeClass.Name == "MessagePackObject"))
                    || ((x.TypeKind == TypeKind.Struct) && x.GetAttributes().Any(x2 => x2.AttributeClass.Name == "MessagePackObject"))
                    )
                .ToArray();
